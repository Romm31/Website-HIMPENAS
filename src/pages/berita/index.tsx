...
import BeritaCard from "@/components/BeritaCard"; // ✅ pakai komponen baru
      ...
      <main className="flex-grow -mt-32">
        <div className="container mx-auto px-4 relative z-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mt-12">
            {berita.length > 0 ? (
              berita.map((item) => <BeritaCard key={item.id} berita={item} />)
            ) : (
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  const [berita, totalBerita, kategoriList] = await Promise.all([
    prisma.berita.findMany({
      ...
      include: { kategori: true }, // ✅ biar ada kategori
    }),
