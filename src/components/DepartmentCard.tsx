      ...
      <div className="relative bg-white rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-500 overflow-hidden border border-gray-100 h-full flex flex-col">
        ...
        <div className="p-6 md:p-8 flex flex-col flex-grow relative">
          ...
          <div className="flex flex-col items-center gap-6 mb-8 flex-grow">
            <motion.div whileHover={{ scale: 1.05 }} className="flex flex-col items-center relative">
                ...
                <motion.div key={i} whileHover={{ scale: 1.05 }} className="flex flex-col items-center">
                  <div className="relative">
                    ...
                    <div className={`relative w-16 h-16 md:w-20 md:h-20 rounded-full overflow-hidden border-4 border-white shadow-lg bg-gray-100 group-hover:border-${color}-400 transition-colors duration-500`}>
                      <Image src={person.imageUrl} alt={person.name} fill sizes="64px" className="object-cover group-hover:scale-110 transition-transform"/>
                    </div>
                  ...
                  <p className="text-xs md:text-sm font-semibold text-gray-800 mt-2 text-center">{person.name}</p>
                  <span className="text-[10px] md:text-xs text-gray-600">{role}</span>
                </motion.div>
              ))}
            </div>
          </div>
          ...
          <div className="mt-auto">
            <motion.div
              ...
              className="flex items-center justify-center gap-2 md:gap-3 p-3 bg-gradient-to-r from-emerald-50 to-emerald-100 rounded-lg group-hover:from-emerald-100 group-hover:to-emerald-200 transition-all"
            >
              <div className="p-2 bg-white rounded-lg shadow-sm group-hover:scale-110 transition-transform">
                <Users className="w-4 h-4 text-emerald-himp" />
              </div>
              <div className="text-left">
                <div className="text-[9px] text-gray-600">Total Anggota</div>
                <div className="text-lg font-bold text-emerald-dark">{staffCount} Staff</div>
              </div>
            </motion.div>
          </div>
        </div>
